    ...
    <div className={classes.root}>
      <div className={classes.form}>
        <aside className={classes.aside}>
          <p className={classes.label}>{labels?.addTextLabel ?? 'Add your text here'}</p>
          <textarea className={classes.textarea} onChange={onChangeText} rows={7} />
          <p className={classes.label}>{labels?.changeColorLabel ?? 'Change color'}</p>
          <ul className={classes.backgroundList}>
            {BACKGROUND_LIST.map((color: string) => {
              return <ColorOption color={color} isActive={color === background} onClick={setBackground} key={color} />;
            })}
          </ul>
        </aside>
...
const ColorOption = React.memo(
  ({ onClick, isActive, color }: { onClick: (color: string) => void; isActive: boolean; color: string }) => {
